...
Code adapted from 
    ...
    https://github.com/google/jax/blob/main/examples/control_test.py
...
class ControlExampleTest(jtu.JaxTestCase):
    def testTrajectoryCyclicIntegerCounter(self):
        ...
        U = jnp.ones((T, 1))
        X = control.trajectory(dynamics, U, jnp.zeros(1), params=None)
    ...
    def testTrajectoryCyclicIndicator(self):
        ...
        X = control.trajectory(
            dynamics, U, jnp.eye(num_states, dtype=jnp.int32)[0], params=None
        )
    ...
    def testLQRSolve(self):
        dim, T = 2, 10
        problem = one_step_lqr(dim, T)
        K, k = control.lqr_solve(problem)
    ...
    def testLQRPredict(self):
        ...
        self.assertAllClose(U[0], -x0, check_dtypes=True, atol=1e-6, rtol=1e-6)
        self.assertAllClose(
            X[1:], jnp.zeros((T, 2)), check_dtypes=True, atol=1e-6, rtol=1e-6
        )
        self.assertAllClose(
            U[1:], jnp.zeros((T - 1, 2)), check_dtypes=True, atol=1e-6, rtol=1e-6
        )
    ...
    def testIlqrWithLQRProblem(self):
        ...
        x0 = randn(dim)
        X, U = control.ilqr(num_iters, p, x0, jnp.zeros((T, dim)), lqr)
        self.assertAllClose(X[0], x0, check_dtypes=True)
    ...
    def testIlqrWithLQRProblemSpecifiedGenerally(self):
        ...
        x0 = randn(dim)
        X, U = control.ilqr(num_iters, p, x0, jnp.zeros((T, dim)), params=None)
        self.assertAllClose(X[0], x0, check_dtypes=True)
        self.assertAllClose(U[0], -x0, check_dtypes=True)
        self.assertAllClose(X[1:], jnp.zeros((T, 2)), check_dtypes=True)
        self.assertAllClose(U[1:], jnp.zeros((T - 1, 2)), check_dtypes=True)
    ...
    def testIlqrWithNonlinearProblem(self):
        def cost(t, x, u, _):
            return (x[0] ** 2.0 + 1e-3 * u[0] ** 2.0) / (t + 1.0)

        def dynamics(t, x, u, _):
            return (x ** 2.0 - u ** 2.0) / (t + 1.0)

        T, num_iters, d = 10, 7, 1
        p = control.ControlSpec(cost, dynamics, T, d, d)

        x0 = jnp.array([0.2])
        X, U = control.ilqr(num_iters, p, x0, 1e-5 * jnp.ones((T, d)), params=None)
        assert_close = partial(self.assertAllClose, atol=1e-2, check_dtypes=True)
        assert_close(X[0], x0)
        assert_close(U[0] ** 2.0, x0 ** 2.0)
    ...
    def testMpcWithLQRProblem(self):
        ...
        X, U = control.mpc_predict(solver, p, x0, jnp.zeros((T, dim)), lqr)
        self.assertAllClose(X[0], x0, check_dtypes=True)
        self.assertAllClose(U[0], -x0, check_dtypes=True)
        self.assertAllClose(X[1:], jnp.zeros((T, 2)), check_dtypes=True)
        self.assertAllClose(U[1:], jnp.zeros((T - 1, 2)), check_dtypes=True)
    ...
    def testMpcWithLQRProblemSpecifiedGenerally(self):
        ...
        x0 = randn(dim)
        solver = partial(control.ilqr, num_iters)
        X, U = control.mpc_predict(solver, p, x0, jnp.zeros((T, dim)), params=None)
        self.assertAllClose(X[0], x0, check_dtypes=True)
        self.assertAllClose(U[0], -x0, check_dtypes=True)
        self.assertAllClose(X[1:], jnp.zeros((T, 2)), check_dtypes=True)
        self.assertAllClose(U[1:], jnp.zeros((T - 1, 2)), check_dtypes=True)
    ...
    def testMpcWithNonlinearProblem(self):
        ...
        x0 = jnp.array([0.2])
        solver = partial(control.ilqr, num_iters)
        X, U = control.mpc_predict(solver, p, x0, 1e-5 * jnp.ones((T, d)), params=None)
        assert_close = partial(self.assertAllClose, atol=1e-2, check_dtypes=True)
        assert_close(X[0], x0)
        assert_close(U[0] ** 2.0, x0 ** 2.0)
        assert_close(X[1:], jnp.zeros((T, d)))
