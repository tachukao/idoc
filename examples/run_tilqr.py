...
def lqr_cost(X, U, theta: dilqr.tilqr.Params):
    Q = theta.lqr.Q
    x0 = theta.x0
...
def init_lqr(key, state_dim, control_dim) -> dilqr.tilqr.TILQR:
    Q = jnp.eye(state_dim)
    ...
    return dilqr.tilqr.TILQR(Q, Qf, R, A, B)


def init_params(key, state_dim, control_dim) -> dilqr.tilqr.Params:
    """Initialize random parameters."""
    key, subkey = jax.random.split(key)
    x0 = jax.random.normal(subkey, (state_dim,))
    key, subkey = jax.random.split(key)
    lqr = init_lqr(subkey, state_dim, control_dim)
    return dilqr.tilqr.Params(x0, lqr)


def check_kkt(kkt: Callable, s: dilqr.typs.State, theta: dilqr.tilqr.Params) -> None:
...
def main(argv):
    ...
    # initialize solvers
    solve_direct, kkt, solve_implicit = dilqr.tilqr.build(T)
